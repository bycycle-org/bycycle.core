...
class RouteService(AService):
    ...
    def query(self, q, points=None):
        ...
        for start, end in zip(starts, ends):
            if start.geom == end.geom:
                ...
                route = Route(
                    start, end, [], LineString([coords, coords]), self.distance_dict(0)
                )
            else:
    ...
    def find_path(
        ...
        add_between = (
            isinstance(start, Street) and isinstance(end, Street) and start.id == end.id
        )
        ...
        assert nodes[0] == start.id, (
            f"Expected route start node ID: {start.id}; got {nodes[0]}"
        )
        assert nodes[-1] == end.id, (
            f"Expected route end node ID: {start.id}; got {nodes[-1]}"
        )
    ...
    def make_directions(self, node_ids, edge_ids, split_edges):
        ...
        for edge, next_edge, next_node_id in zip(
            edges, chain(edges[1:], [None]), node_ids[1:]
        ):
            name = edge.name
            geom = edge.geom
            length = edge.meters
    ...
    def calculate_way_to_turn(self, old_bearing, new_bearing):
        ...
        if 0 <= diff < 10:
            ...
            raise ValueError(
                "Could not calculate way to turn from {} and {}".format(

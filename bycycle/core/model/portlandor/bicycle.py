...
class TravelMode(tmode.TravelMode):
    ...
    def __init__(self, region, pref='default'):
        """Configure bicycle travel mode.

        ``pref`` `string`
            User's simple preference option. Can be empty or one of
            "default", "flatter", "safer", "shorter", or "faster".

        """
        pref = (pref or 'DEFAULT').upper()
        ...
        def get_cost(u, v, edge_attrs, prev_edge_attrs):
            ...
            # Swap uphill and downhill lengths when traversing edge to
            # => from
            if v != node_f_id:
                up_len, down_len = down_len, up_len

            # Based on the slope, calculate the speed of travel uphill
            # and downhill (up_spd & down_spd)
            if slope <= 0:
